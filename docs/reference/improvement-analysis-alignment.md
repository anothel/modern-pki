...
| Issuance consistency needs failure injection. | `docs/reference/issuance-consistency.md` documents signer claim, retry, and repair behavior; tests cover signer success plus DB finalization failure, retry without second signing, lease races, duplicate certificate finalization keys, and stale issuance-attempt finalization updates. | Add lifecycle-level serial collision tests and broader PostgreSQL parity where memory/SQLite coverage already exists. |
