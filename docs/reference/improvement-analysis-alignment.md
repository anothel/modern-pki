...
| ACME nonce/replay/key binding tests should expand. | ACME malformed JWS, nonce reuse, badNonce retry, KID base URL, key mismatch, account key, key rollover, rate limit, and lego smoke coverage exist. | Add certbot-derived fixtures if certbot live smoke exposes differences. |
