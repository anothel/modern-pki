...
| Certificate profiles | Profile-as-code fields for validity, Basic Constraints, KU, EKU, SAN, SKI, AKI; issued DER golden tests assert SAN, KU, EKU, Basic Constraints, AIA, CRL Distribution Points, SKI, and AKI. | Add profile-level key/signature algorithm policy. |
...
| CSR/certificate correctness needs stronger tests. | Profile policy, public TLS policy, and issued DER golden tests exist. | Add CSR linting, profile algorithm policy, and remaining negative certificate policy cases. |
