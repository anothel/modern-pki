...
| NIST SP 1800-16 | Monitoring and response | Partial | Audit metadata with request, trace, auth, API key fingerprint, state transition, redacted secret handling, query pagination, and retention pruning; structured startup logs, HTTP boundary metrics, DB/signer/core CLI operation metrics, recovery runbook, incident response runbook, webhook/outbox safety runbook. | Metrics exporter, distributed span backend, tamper-evident audit storage, and SIEM export remain roadmap items. |
